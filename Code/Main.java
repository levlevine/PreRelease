...
public class Main {
    ...
    int checkMultiplyer(String word)
    {
	    int multiplyer = 0;
    ...
    String removeBonus(String word)
    {
    	word = word.replace("$", "");
    	word = word.replace("£", "");
		return(word);
    ...
    void updateAfterAllowedWord(String word, Tiles playerTiles, 
            ...
            String[] allowedWords, int multiplyer)
    {
            ...
            Map tileDictionary)
    {
        ...
        {
        	if (playerTiles.charAt(count) == '£'|| playerTiles.charAt(count) =='$') 
        	{
        	}
        	else 
            ...
            int noOfEndOfTurnTiles)
    {
      ...
      while (!validChoice)
      {
        ...
        else
        {
          ...
          if (validWord)
          {
        	int multiplyer = checkMultiplyer(choice);
        	if (multiplyer > 0)
        	{
        		choice = removeBonus(choice);
        	}
            ...
            if (validWord)
            {
              ...
              updateAfterAllowedWord(choice, playerTiles, playerScore, 
                      playerTilesPlayed, tileDictionary, allowedWords, multiplyer);
